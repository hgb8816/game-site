...
def aff_box(kw):
    return (
        ...
        '    <br><a href="REPLACE_WITH_JD_UNION_LINK" target="_blank" rel="nofollow">查看京东%s精选 ›</a>\n'
        '    <br><span style="color:var(--sub);font-size:12px">你的京东联盟 PID：2038574972_4108035044_3108499673｜ 去 <a href="https://union.jd.com" target="_blank" rel="nofollow">京东联盟后台</a> → 我要推广 → 商品推广 → 搜「%s」→ 选推广位 PID=4108035044 → 复制 https://u.jd.com/xxxxxx 短链替换上面占位即可生效。</span>\n'
        '  </div>\n\n' % (kw, kw)
    )

def build(title, desc, tag, readmin, body, affiliate):
    return (
        '<!doctype html>\n'
        '<html lang="zh-CN">\n'
        '<head>\n'
        ...
        '</body>\n'
        '</html>\n'
    ) % (title, desc, title, tag, readmin, body, affiliate)

articles = {}

articles["ps5-vs-xbox-series-x.html"] = build(
    "PS5 还是 Xbox Series X：2026 年怎么选",
    "2026 年 PS5 与 Xbox Series X 怎么选？从独占游戏、性能、订阅服务、二手生态四个维度对比，给出结论。",
    "对比", "6 分钟",
...
articles["pc-games-2026.html"] = build(
    "2026 年值得玩的 PC 单机游戏（按类型挑）",
    "2026 年 PC 单机游戏推荐：开放世界、Roguelike、剧情向、模拟经营四类精选，附适合人群。",
    "推荐", "5 分钟",
...
articles["best-gaming-monitor-2026.html"] = build(
    "2026 游戏显示器怎么选：刷新率/面板/尺寸一篇讲清",
    "2026 年游戏显示器选购指南：144Hz/240Hz、IPS/VA/OLED、27 寸怎么选，按用途与预算给结论。",
    "外设导购", "5 分钟",
...
articles["steam-lowest-price-tools.html"] = build(
    "Steam 史低查询与降价提醒：这几招帮你省一半",
    "Steam 怎么查史低价格、设降价提醒、判断该不该买？推荐常用工具与判断方法，原创实用向。",
    "省钱", "4 分钟",
...
articles["gpu-2026-buy-guide.html"] = build(
    "2026 显卡怎么选：按预算与用途闭眼入",
    "2026 年显卡选购指南：1080P/2K/4K 三档对应型号、显存怎么看、新手避坑，附联盟清单。",
    "高客单", "6 分钟",
...
articles["pc-build-5000-8000-12000.html"] = build(
    "5000/8000/12000 三档电脑配置单（2026 版）",
    "2026 年三档台式机配置单：5000 入门、8000 甜点、12000 高端，含显卡优先级与避坑。",
    "高客单", "7 分钟",
...
articles["mobile-to-pc-games.html"] = build(
    "手游玩家转 PC 单机：按你喜欢的手游类型挑",
    "从手游转 PC 单机玩什么？按 MOBA/卡牌/开放世界/音游四类，平滑过渡到合适的 PC 游戏。",
    "推荐", "5 分钟",
...
articles["ssd-vs-hdd-game-storage.html"] = build(
    "游戏存储：SSD 还是 HDD？2026 年怎么搭配最划算",
    "游戏装 SSD 还是 HDD？加载速度、容量、价格的取舍，给出 2026 年性价比搭配方案。",
    "对比", "4 分钟",
...
for name, html in articles.items():
    with open(os.path.join(art, name), "w", encoding="utf-8") as f:
        f.write(html)
    print("已生成:", name, "(", len(html), "字符 )")
print("=== 第二批共 %d 篇生成完成 ===" % len(articles))
